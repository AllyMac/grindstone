...
<template>
  <div class="flex flex-col gap-3">
    <div class="flex items-center justify-between">
      <h2 class="text-sm font-semibold text-stone-700">Which PC is yours?</h2>
      <button v-if="canCancel" type="button" class="text-sm text-stone-500 hover:underline" @click="emit('cancel')">
        ← Back
      </button>
    </div>

    <p v-if="unclaimed.length === 0" class="text-sm text-stone-400">No unclaimed PCs - make yours below.</p>
    ...
    <button
      type="button"
      class="rounded-md bg-stone-700 px-3 py-1.5 text-sm font-medium text-white hover:bg-stone-800"
      @click="emit('create')"
    >
      + Create a new PC
    </button>
